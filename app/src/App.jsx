...
export function App() {
  ...
  useEffect(() => {
    const handle = () => setRoute(location.pathname);
  ...
  useEffect(() => {
    ...
    try { localStorage.setItem('melodik.volume', JSON.stringify(volume)); } catch {}
  }, [volume]);
  useEffect(() => {
  ...
  useEffect(() => {
    setElapsed(0);
    setDuration(0);
    if (current && shouldPlay.current) {
      audio.current.play().catch(() => notify('Chạm nút phát để bắt đầu nghe nhạc.'));
    }
  }, [current?.audio, notify]);

  // Admin auth check
  const checkAdminAuth = () => api('/api/auth/me').then(setAdminAuth).catch(error => setAdminAuthError(error.message));
  useEffect(() => {
    if (route === '/admin') {
      checkAdminAuth();
    }
  ...
  const toggle = useCallback(() => {
    ...
    if (audio.current.paused) {
      ...
      audio.current.play().catch(() => notify('Không phát được file này. Vui lòng thử bài khác.'));
    } else {
      shouldPlay.current = false;
      audio.current.pause();
    }
  ...
  const skip = useCallback((direction = 1, ended = false) => {
    ...
    if (direction < 0 && audio.current.currentTime > 3) {
      audio.current.currentTime = 0;
      return;
    }
    ...
    shouldPlay.current = true;
    if (queue[next].id === currentId) {
      audio.current.currentTime = 0;
      audio.current.play().catch(() => {});
    } else setCurrentId(queue[next].id);
  }, [allCatalogTracks, current, currentId, repeat, shuffle]);

  useEffect(() => {
    const handle = e => {
      if (e.code === 'Space' && !e.repeat && !e.target.closest('input,textarea,button,a,select,[contenteditable],dialog')) {
        e.preventDefault();
        toggle();
      }
    };
  ...
  function favorite(trackId) {
    setFavorites(old => {
      const next = old.includes(trackId) ? old.filter(v => v !== trackId) : [...old, trackId];
      try { localStorage.setItem('melodik.favorites', JSON.stringify(next)); } catch {}
      return next;
    });
  }
    ...
    <>
      ...
      <header className="site-header">
        <div className="header-inner">
            ...
            {isAdmin ? (
              <>
                ...
                <a
                  ...
                  aria-current={activeTab === 'messages' ? 'true' : undefined}
                >
                  Tin nhắn
                </a>
              </>
              ...
              <>
                {link('/music', 'Góc nghe nhạc')}
                {link('/gallery', 'Kỷ niệm')}
                {link('/community', 'Góc của bạn')}
                {link('/guestbook', 'Sổ lưu bút')}
              </>
            )}
          </nav>
          {/* Profile / Auth link */}
          {isAdmin ? (
            <div
              className={`hello-link ${adminAuth ? 'profile-link' : ''}`}
              onClick={e => {
                e.stopPropagation();
            ...
            >
              {adminAuth ? (
                <>
                  <img src={adminAuth.avatar || '/artwork/sleeve.webp'} alt="" />
                  <span>{adminAuth.name}</span>
                </>
              ) : (
                <NotePencil size={17} />
              )}
              {adminAuth ? <span>{adminAuth.name}</span> : 'Admin'}
              <ArrowUpRight size={15} />
            </div>
          ) : (
            <a
              className={`hello-link ${me ? 'profile-link' : ''}`}
              href="/community"
              onClick={e => {
                e.preventDefault();
                navigate('/community');
            ...
            >
              {me ? <img src={me.avatar || '/artwork/sleeve.webp'} alt="" /> : <NotePencil size={17} />}
              {me ? <span>{me.name}</span> : 'Đăng ký / Đăng nhập'}
              <ArrowUpRight size={15} />
            </a>
          )}
        </div>
      ...
      <main id="main" className={isAdmin ? 'admin-main' : 'main-content'}>
        {loadError ? (
          <div className="empty-state">
            <h2>Góc nhạc chưa kết nối được.</h2>
            <p>{loadError}</p>
            <button className="button primary" onClick={refresh}>Thử lại</button>
          </div>
        ) : !catalog ? (
          <div className="empty-state">
            <CircleNotch className="spin" size={32} />
            <p>Đang mở góc nhạc của bạn…</p>
          </div>
        ) : isAdmin ? (
          <Admin
            catalog={catalog}
          ...
          <Community catalog={catalog} refresh={refresh} notify={notify} me={me} onUserChange={refreshCurrentUser} personalSettings={userSettings} onPersonalSettingsChange={refreshUserSettings} />
        ) : route === '/guestbook' ? (
          <Guestbook settings={{ ...(catalog.settings || {}), ...userSettings }} />
        ) : ['/', '/music'].includes(route) ? (
          <Music catalog={catalog} player={player} me={me} personalSettings={userSettings} onPersonalSettingsChange={refreshUserSettings} />
        ) : (
          <div className="empty-state">
            <h1>Trang này đi lạc rồi.</h1>
            <button className="button primary" onClick={() => navigate('/')}>Về góc nghe nhạc</button>
          </div>
        )}
      </main>
      ...
      <footer className="site-footer">
        <div>
          <a className="wordmark small" href="/" onClick={e => { e.preventDefault(); navigate('/'); }}>
            MIUZIG
          </a>
          <p>Một chút nhạc. Một chút bình yên.</p>
        </div>
        <span className="footer-note">Made for slow days <Heart size={14} /></span>
        <a href="/admin" onClick={e => { e.preventDefault(); navigate('/admin'); }}>Quản trị <ArrowUpRight size={14} /></a>
      </footer>

      {!['/', '/music'].includes(route) && current && (
        miniPlayerCollapsed ? (
          <button
          ...
          <div className="mini-player">
            <img src={currentCover || current.cover} alt="" />
            <div className="mini-title">
              <strong>{current.title}</strong>
              <span>{current.artist || current.owner}</span>
            </div>
            <div className="mini-controls">
              <IconButton icon={SkipBack} label="Bài trước" onClick={() => skip(-1)} />
              <IconButton className="primary" icon={playing ? Pause : Play} label={playing ? 'Tạm dừng' : 'Phát nhạc'} onClick={toggle} />
              <IconButton icon={SkipForward} label="Bài tiếp theo" onClick={() => skip(1)} />
            </div>
            <input
              ...
              value={Math.min(elapsed, duration || current.duration)}
              step=".1"
              onChange={e => seek(e.target.value)}
            />
            <span className="mono">{time(elapsed)}</span>
            <button className="mini-return" onClick={() => navigate('/music')}>
              Góc nhạc
      ...
      {toast && (
        <div role="status" className="toast show">
          {toast}
          <IconButton icon={X} label="Đóng thông báo" onClick={() => setToast('')} />
        </div>
      )}
    </>
  );
