...
import { ArrowsClockwise, ChatCircleText, Check, CircleNotch, Eye, EyeSlash, FileArrowUp, Heart, Images, MusicNotes, PaperPlaneTilt, PencilSimple, SignOut, Smiley, UserCircle, UserPlus, UsersThree, X } from '@phosphor-icons/react';
...
export function Community({ catalog, refresh, notify, me: appUser, onUserChange, onPersonalSettingsChange }) {
  ...
  const [search, setSearch] = useState(''), [results, setResults] = useState([]), [replyTo, setReplyTo] = useState(null), [forwarding, setForwarding] = useState(null);
  const [genreDraft, setGenreDraft] = useState(''), [categoryDraft, setCategoryDraft] = useState('');
  const [registerId, setRegisterId] = useState(randomPublicId), fileRef = useRef(null);
  ...
  async function sendChat(event) {
    ...
    try {
      ...
      if (forwarding) form.set('forwardedFrom', forwarding.id);
      await api(`/api/chat/${selectedFriend.id}`, { method: 'POST', body: await uploadFiles(form, storage, [['attachment', 'chat']]) });
      formEl.reset(); setReplyTo(null); setForwarding(null); await loadChat(selectedFriend);
    } catch (e) { setError(e.message); } finally { setBusy(false); }
  }
