    ...
    {composer === 'camera' && <Modal title={cameraStep === 'details' ? 'Thêm nội dung kỷ niệm' : 'Chụp một kỷ niệm'} onClose={closeCameraComposer} className={`camera-modal ${isMobileCamera ? 'camera-mobile-flow' : ''} camera-step-${cameraStep}`}>
      <form className="editor-form camera-form" onSubmit={submitCameraPhoto}>
        ...
        {(!isMobileCamera || cameraStep === 'details') && <div className="camera-details-panel">
          ...
          <label>Danh mục tự thêm<input name="category" required maxLength={50} value={categoryDraft} onChange={e => setCategoryDraft(e.target.value)} placeholder="Ví dụ: du lịch, bạn bè, sinh nhật..." autoComplete="off" onFocus={() => setCategoryFocus(true)} onBlur={() => setCategoryFocus(false)} /></label>
          <SuggestionChips items={categorySuggestions} onPick={v => { setCategoryDraft(v); setCategoryFocus(false); }} visible={categoryFocus} />
