...
async function deleteChatMessage(messageId) {
  ...
  <div className="message-time">{new Date(message.createdAt).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}, {new Date(message.createdAt).toLocaleDateString('vi-VN')}</div>
    ...
    {composer === 'photo' && <Modal title="Đăng một kỷ niệm" onClose={() => !busy && setComposer(null)}><form className="editor-form" onSubmit={submitPhoto}><label>Tiêu đề<input name="title" required maxLength={120} /></label><label>Danh mục tự thêm<input name="category" required maxLength={50} value={categoryDraft} onChange={e => setCategoryDraft(e.target.value)} placeholder="Ví dụ: du lịch, bạn bè, sinh nhật..." autoComplete="off" /></label><SuggestionChips items={categorySuggestions} onPick={setCategoryDraft} /><label>Hiển thị<select name="visibility" defaultValue="self"><option value="self">Chỉ mình tôi</option><option value="friends">Bạn bè đã kết bạn</option></select></label><label>Ảnh<input name="image" type="file" accept={imageAccept} required /></label><label>Địa điểm<input name="location" maxLength={150} /></label><label>Câu chuyện nhỏ<textarea name="caption" maxLength={1000} rows={3} /></label><label>Ngày<input name="date" type="date" required defaultValue={new Date().toISOString().slice(0, 10)} /></label><button className="button primary wide" disabled={busy}>{busy ? 'Đang đăng…' : 'Chia sẻ kỷ niệm'}</button></form></Modal>}
