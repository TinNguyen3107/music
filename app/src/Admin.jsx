...
export function Admin({ notify, navigate }) {
 ...
 if (!auth) return <div className="empty-state">{authError ? <><p role="alert">{authError}</p><button className="button secondary" onClick={check}>Thử lại</button></> : <CircleNotch size={30} className="spin" />}</div>;
  ...
  return (
    <>
     <header className="site-header"><div className="header-inner"><a className="wordmark" href="/" onClick={e => { e.preventDefault(); navigate('/'); }}>MIUZIG<span className="logo-star"><Sparkle size={18} weight="fill" /></span></a><nav aria-label="Điều hướng chính"><a href="/music" onClick={e => { e.preventDefault(); navigate('/music'); }}>Góc nghe nhạc</a><a href="/gallery" onClick={e => { e.preventDefault(); navigate('/gallery'); }}>Kỷ niệm</a><a href="/community" onClick={e => { e.preventDefault(); navigate('/community'); }}>Góc của bạn</a><a href="/guestbook" onClick={e => { e.preventDefault(); navigate('/guestbook'); }}>Sổ lưu bút</a></nav><a className="hello-link profile-link" href="/community" onClick={e => { e.preventDefault(); navigate('/community'); }}>{auth ? <img src={auth.avatar || '/artwork/sleeve.webp'} alt="" /> : <NotePencil size={17} />}{auth ? <span>{auth.name}</span> : 'Admin'}<ArrowUpRight size={15} /></a></div></header>
