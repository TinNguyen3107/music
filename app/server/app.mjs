...
export async function createApp({ dataDir = process.env.DATA_DIR || path.join(appRoot, 'data'), seedData = true, seedDefaultAccounts = true, production = process.env.VERCEL === '1' } = {}) {
  ...
  const userSession = req => row('SELECT users.id,users.name,users.email,users.publicId,users.avatar FROM user_sessions JOIN users ON users.id=user_sessions.userId WHERE user_sessions.token=? AND user_sessions.expires>?', [digest(userTokenFrom(req)), Date.now()]);
  ...
  async function registerUser(req, res) {
    ...
    await query('INSERT INTO users (id,name,email,publicId,avatar,passwordHash,salt,createdAt) VALUES (?,?,?,?,?,?,?,?)', [userId, name, mail, userPublicId, avatar, scryptSync(password, salt, 64).toString('hex'), salt, new Date().toISOString()]);
  ...
  app.post('/api/users/login', async (req, res) => {
    ...
    await createUserSession(res, user.id); res.json({ user: { id: user.id, name: user.name, email: user.email, publicId: user.publicId, avatar: user.avatar } });
  });
  ...
  app.get('/api/friends', authUser, async (req, res) => res.json(await all("SELECT users.id,users.name,users.publicId,users.avatar,friendships.status,CASE WHEN friendships.userId=? THEN 'outgoing' ELSE 'incoming' END AS direction FROM friendships JOIN users ON users.id=CASE WHEN friendships.userId=? THEN friendships.friendId ELSE friendships.userId END WHERE friendships.userId=? OR friendships.friendId=? ORDER BY friendships.createdAt DESC", [req.user.id, req.user.id, req.user.id, req.user.id])));
  ...
  app.get('/api/chat/:id', authUser, async (req, res) => { if (!await areFriends(req.user.id, req.params.id)) throw fail('B?n ch? c� th? tr� chuy?n v?i b?n b�.', 403); res.json(await all('SELECT c.*, r.attachment AS replyAttachment, r.message AS replyMessage, r.attachmentName AS replyAttachmentName, r.senderId AS replySenderId FROM chat_messages c LEFT JOIN chat_messages r ON c.replyTo = r.id WHERE (c.senderId=? AND c.recipientId=?) OR (c.senderId=? AND c.recipientId=?) ORDER BY c.createdAt ASC LIMIT 200', [req.user.id, req.params.id, req.params.id, req.user.id])); });
